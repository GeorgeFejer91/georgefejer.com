...
1. Runtime IDs for the four profiles: `lowLow`, `lowHigh`, `highLow`,
   `highHigh`, or mapped `induction_a` through `induction_d`.
2. Whether scenario order is fixed as ambient/background first and hand-avatar
   second, or counterbalanced.
...
can instantiate a valence-arousal state space. Four particle profiles cross
...
H3: the four profiles will occupy the intended relative valence-arousal states:
...
Primary subjective outcomes are valence and arousal on 0-100 sliders. SAM
valence and SAM arousal, each rated 1-9, are convergent outcomes; SAM
dominance/control is exploratory.
...
Onboarding/demographic fields include language, age, handedness, gender,
...
The design is fully within-participant. Twenty participants complete two
mapping scenarios: ambient/background particle mapping and hand-avatar
texture/material mapping. In each scenario, participants experience all four
particle-motion profiles:
...
Within each scenario, the four profiles are presented in randomized,
counterbalanced order, using constrained randomization so profiles appear
approximately equally often in each serial position across the target sample.
...
Confirmatory analyses use block-level data with one row per participant,
scenario, and profile. Coherence is coded low/desynchronized vs
high/synchronized; energy is coded low vs high; scenario is coded
ambient/background vs hand-avatar. Mixed-effects models include participant as a
random intercept; random slopes for within-participant factors will be included
if they converge.

H1: model valence slider ratings from scenario, coherence, energy, and their
interactions. The planned contrast is high coherence > low coherence, averaged
over energy and scenario. Repeat for SAM valence.

H2: model arousal slider ratings with the same predictors. The planned contrast
is high energy/noise > low energy/noise, averaged over coherence and scenario.
Repeat for SAM arousal and for the preregistered ECG/HRV arousal index or
feature family.

H3: evaluate whether the four profile means match the intended quadrants by
combining the H1 valence contrast, H2 arousal contrast, and descriptive
profile-level valence-arousal means.

H4: model ownership and agency from valence, arousal, scenario, coherence, and
...
demographics, and scenario interactions.
...
consent, withdraw, do not complete both scenarios and required assessments, are
ineligible under the final recruitment criteria `[TBD: eligibility criteria]`,
or have a major technical failure preventing linkage of scenario, profile, or
order to outcomes.

Exclude a condition block, while retaining other valid blocks where possible,
if required outcomes are missing, data are malformed, scenario/profile IDs are
invalid, language-matched audio is incorrect, or a documented runtime failure
occurs during induction or assessment.
...
up to eight block-level observations: 2 mapping scenarios x 4 particle-motion
profiles, for a maximum of 160 block-level observations.
...
parameters should be held constant where possible across profiles and
scenarios.
...
If scenario order remains fixed as ambient/background first and hand-avatar
second, scenario effects will be interpreted in relation to that two-part
order. If scenario order is counterbalanced, this preregistration should be
