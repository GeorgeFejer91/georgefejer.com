...
The public package includes cached prompt MP3s in `prompt_audio_raw/`. These let future work rebuild the final files without spending additional ElevenLabs quota, as long as the exact prompt wording remains unchanged.
...
- Source wording files in `source_transcripts_from_zip/`.
- Exact final timed transcripts in `transcripts/`.
- Cue-level wording and timing in `timing_library.csv`.
...
```powershell
cd Research/study-6/neutral-hand-audio
