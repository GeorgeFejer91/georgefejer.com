...
Maintain and continue the Study 6 neutral ambient hand-task audio package. The package provides 4 English and 4 German guided audio files, each exactly 5 minutes long, with standardized timing and different action-cluster orders. These are participant-heard instruction scripts for integration into the Study 6 questionnaire/experiment workflow.
...
- Exact wording library: `wordings/`.
...
- Treat the four versions as variants of the same instruction with different
  movement orders.
- Randomly assign the four variants across the four blocks for a
  participant/session.
- The exact shuffle method is not critical; the goal is random block-level
  assignment so repeated instructions feel less repetitive.
...
- `protocol/`: research goals, full context, protocol, asset map, and validation checklist.
- `wordings/`: exact wording library plus source and final transcript copies for research-facing review.
- `for-ai/`: project-local AI/operator notes. Keep new one-off helpers there unless they are part of the reproducible audio package.
...
Copy this entire `Research/study-6/neutral-hand-audio/` folder into the other project when continuing the work. Keep the relative structure intact.
