...
- Evil-frog corruption must branch from the actual visible unicorn/princess morph row, not merely from `playerForm`. A player caught while partially transformed should begin the witch transition from that same continuum state. `hero-witch-corruption-walk.png` reserves rows 0-13 for princess/unicorn-to-corruption branch starts and rows 14-17 for final takeover into the full green witch; row 17 must resolve to the actual `green-witch-run.png` identity.
