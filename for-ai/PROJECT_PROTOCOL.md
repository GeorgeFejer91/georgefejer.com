...
- Evil-frog corruption must branch from the actual visible unicorn/princess morph row, not merely from `playerForm`. A player caught while partially transformed should begin the witch transition from that same continuum state. `hero-witch-corruption-walk.png` is an 18 x 8 transformation atlas: rows 0-17 are the current unicorn/princess morph source states, and columns 0-7 are the staged transformation into a green witch. Do not use a crossfade or double-exposure overlay for this; each cell should read as one coherent intermediate body with magic transformation arches, and column 7 must resolve to the actual `green-witch-run.png` identity.
