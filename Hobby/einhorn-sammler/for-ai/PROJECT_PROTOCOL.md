...
Future agents must read this file before changing `Hobby/einhorn-sammler/index.html`.
...
All project-specific orchestrator/operator scripts, validation harnesses, prompt runners, and one-off agent helpers for this game must live in `Hobby/einhorn-sammler/for-ai/` or a child folder inside it.
Before changing sprite sheets, also read `SPRITE_CONSISTENCY_CHECKLIST.md`.
Before creating, regenerating, or repairing transformation sprites, also read `SPRITE_TRANSFORMATION_PROCEDURES.md`.
After changing sprite sheets, update the latest sprite audit log.

## Current Project Goal

Build a smartphone-playable 2D fantasy maze chase game at `/Hobby/einhorn-sammler/` on GeorgeFejer.com.
...
- Evil-frog corruption must branch from the actual visible unicorn/princess morph row, not merely from `playerForm`. A player caught while partially transformed should begin the witch transition from that same continuum state. `hero-witch-corruption-walk.png` is an 18 x 18 transformation atlas: rows 0-17 are the current unicorn/princess morph source states, and columns 0-17 are the staged transformation into a green witch. Do not use a crossfade or double-exposure overlay for this; each cell should read as one coherent intermediate body with magic transformation arches. The late columns must keep a continuous body size, anchor, and silhouette instead of inserting a smaller green-witch endpoint.
- Witch broomstick power-ups must render with `witch-broom-flight.png`, an original generated 16-frame broom-flight sprite where the witch is visibly seated astride the broom in every frame. The broom tail must not be clipped, frames need wide transparent gutters so neighboring poses do not visually bleed into each other, and playback should stay smooth enough for phone browsers.
...
- Debug and validation should inspect the canvas dataset fields `assetVersion`, `preloadMode`, `assetPhase`, `assetWebp`, `assetPngFallback`, `assetRawFallback`, `assetDecoded`, `assetTotal`, `currentSprites`, `currentHeroMorphRow`, `witchCorruptionStartRow`, `witchCorruptionHeroRow`, `sfxReady`, `musicReady`, `musicRawFallback`, and `audioReady`.
...
When browser automation cannot call page globals, use `/Hobby/einhorn-sammler/?aitest=1&runs=3&seconds=100&tick=55` and read `#difficulty-report`.
...
`https://www.georgefejer.com/Hobby/einhorn-sammler/`

If a change is pushed, verify the live page reports the new `assetVersion` through the canvas dataset or debug hook.
Also verify that `https://www.georgefejer.com/Hobby/einhorn-sammler/` passes normal browser/curl certificate validation; if it does not, the repository code may be deployed correctly but browsers can still display safety warnings.
